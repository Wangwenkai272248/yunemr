        ...
        for (int m = 0; m < listObject.size(); m++) {
            ...
            for (int n = 0; n < json2List.size(); n++) {
                ...
                if (!"TRUE".equals(bool.trim())) {
                    ...
                    if (n == 2 || n == 3 || n == 4 || n == 6 || n == 14) {
                        String jsonObject = (String) json2List.get(n);
                        for (int i = 0; i < strs1.length; i++) {
//                            if (jsonObject.contains(strs1[i])) {
                            if (jsonObject.contains("\""+strs1[i]+"\"") || jsonObject.contains("住院医嘱_医嘱项名称等于" + strs1[i] + "and") || jsonObject.contains("住院医嘱_医嘱项名称等于" + strs1[i] + ")")) {
                                System.out.println("第" + n + "列需要替换的:" + strs1[i] + "==" + strs2[i]);
                                System.out.println("修改前：" + jsonObject);
                                jsonObject = jsonObject.replaceAll("\""+strs1[i]+"\"", "\""+strs2[i]+"\"").replaceAll("住院医嘱_医嘱项名称等于" + strs1[i] + "and", "住院医嘱_医嘱项名称等于" + strs2[i] + "and").replaceAll("住院医嘱_医嘱项名称等于" + strs1[i] + "\\)", "住院医嘱_医嘱项名称等于" + strs2[i] + "\\)");
