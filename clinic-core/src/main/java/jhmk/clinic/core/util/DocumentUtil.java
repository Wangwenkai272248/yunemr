...
import jhmk.clinic.core.config.UrlConstants;
...
public class DocumentUtil {
    ...
    public String getStandardFromAlias(String name) {
        ...
        String standardName = restTemplate.postForObject("http://192.168.132.13:8805" + UrlConstants.standardFromAlias, request, String.class);
    ...
    public List<String> getAllChildDisease(String name) {
        ...
        postParameters.add("word", trim);
        HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<MultiValueMap<String, String>>(postParameters, header);
        Object allDisease = restTemplate.postForObject("http://192.168.132.13:8805" + UrlConstants.getAllChildDisease, request, Object.class);
    ...
    public List<String> getMedicineFromLevelName(String name) {
        ...
        HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<MultiValueMap<String, String>>(postParameters, header);
        try {
            ArrayList<String> medicineFromLevelName = restTemplate.postForObject("http://192.168.132.13:8805" + UrlConstants.getMedicineFromLevelName, request, ArrayList.class);
            return medicineFromLevelName;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("调用{}方法失败：，请求参数为：{}"+"http://192.168.132.13:8805" + UrlConstants.getMedicineFromLevelName,postParameters);
            return null;
        }
    }
